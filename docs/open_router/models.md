...
> Projeto: priorize os modelos `x-ai/grok-4-fast` (chat) e `google/gemini-2.5-flash-image-preview` (visão) ao consumir esta lista de modelos via OpenRouter.
...
| --- | --- | --- |
| `id` | `string` | Unique model identifier used in API requests (e.g., `"x-ai/grok-4-fast"`) |
...
<Note title='Different models tokenize text in different ways'>
  Some models break up text into chunks de múltiplos caracteres (Grok, Claude,
