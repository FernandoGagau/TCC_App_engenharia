...
> Projeto: configure o OpenRouter como camada padrão, usando o modelo `x-ai/grok-4-fast` para agentes conversacionais e `google/gemini-2.5-flash-image-preview` para fluxos visuais/imagem. Atualize `langgraph.json` ou o servidor local com essas credenciais e headers exigidos pelo OpenRouter.
...
As an example, Grok-4 Fast (compatível com a API da OpenAI) usa um fluxo equivalente ao [save_memories](https://openai.com/index/memory-and-new-controls-for-chatgpt/) para gravar memórias como strings de conteúdo, decidindo quando aplicar a cada mensagem do usuário. Veja o [memory-agent](https://github.com/langchain-ai/memory-agent) como referência.
